...
start_trip() {
  ...
  ros2 launch trip run_corridor_TRIP.py >/dev/null 2>&1 &
  TRIP_PID=$!
}
...
start_s_graphs() {
  ...
  ros2 launch lidar_situational_graphs tiers10_launch.py room_segmentation:=traversable compute_odom:=true \
...
start_rviz() {
  ...
  source /opt/ros/foxy/setup.bash
  source ~/workspaces/s-graph/install/setup.bash
  rviz2 -d ~/workspaces/s-graph/lidar_situational_graphs/rviz/s_graphs_ros2.rviz render_quality:=high &
  #RVIZ_PID=$!
}
...
start_base_tf() {
  ...
  ros2 run tiers lidar_calibrate_2laps & #--ros-args -p use_sim_time:=true &
  BASE_TF_PID=$!
}
...
echo "Maximum iterations reached. Waiting 10s."
sleep 30
echo "Terminating."
stop_existing_node