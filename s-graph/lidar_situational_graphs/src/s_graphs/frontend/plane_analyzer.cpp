...
std::vector<pcl::PointCloud<PointNormal>::Ptr> PlaneAnalyzer::extract_segmented_planes(
  ...
  while (transformed_cloud->points.size() > min_seg_points_) {
    try {
      ...
      // std::cout<<"extracted_cloud_filtered "<<extracted_cloud_filtered->points.size()<<std::endl;

      if ((fabs(coefficients->values[1]) > 0.9 && extracted_cloud_filtered->points.size() > min_horizontal_inliers) 
          || ((fabs(coefficients->values[0]) > 0.9 || fabs(coefficients->values[1]) > 0.9) && extracted_cloud_filtered->points.size() > min_vertical_inliers))
