...
situational_graphs_reasoning_msgs::msg::Graph GraphPublisher::publish_graph(
  ...
  if (graph_type == "Prior") {
    ...
    for (int i = 0; edge_itr != local_graph->edges().end(); edge_itr++, i++) {
      ...
      g2o::EdgePlaneIdentity* edge_pi = dynamic_cast<g2o::EdgePlaneIdentity*>(edge);
      ...
      if (edge_2p) {
          // std::cout<<"edge_2p"<<std::endl;
        v_plane1 = dynamic_cast<g2o::VertexPlane*>(edge_2p->vertices()[0]);
        v_plane2 = dynamic_cast<g2o::VertexPlane*>(edge_2p->vertices()[1]);
      // }
      // else if (edge_pi) {
      //     std::cout<<"edge_pi"<<std::endl;
      //   v_plane1 = dynamic_cast<g2o::VertexPlane*>(edge_pi->vertices()[0]);
      //   v_plane2 = dynamic_cast<g2o::VertexPlane*>(edge_pi->vertices()[1]);
      //     std::cout<<"v_plane1 v_plane2"<<std::endl;
      // }
      // if (v_plane1 && v_plane2) {
        // Plane 1 node
