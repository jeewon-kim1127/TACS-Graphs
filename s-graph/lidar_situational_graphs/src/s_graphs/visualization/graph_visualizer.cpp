...
visualization_msgs::msg::MarkerArray GraphVisualizer::create_marker_array(
  ...
  for (int i = 0; i < x_infinite_room_snapshot.size(); ++i) {
    ...
    x_infinite_room_line_marker.scale.x = 0.1;
    ...
    p1.z = x_infinite_room_snapshot[i].node->estimate().translation()(2);
    ...
    infinite_room_pose_marker.scale.x = 1.0;
    infinite_room_pose_marker.scale.y = 1.0;
    infinite_room_pose_marker.scale.z = 1.0;
    ...
    if (!overlapped_infinite_room) {
      ...
      infinite_room_pose_marker.color.r = 0;
      infinite_room_pose_marker.color.g = 140/255.0;
      infinite_room_pose_marker.color.b = 10/255.0;
      infinite_room_pose_marker.color.a = 1.0;
  ...
  for (int i = 0; i < y_infinite_room_snapshot.size(); ++i) {
    ...
    y_infinite_room_line_marker.scale.x = 0.1;
    ...
    p1.z = y_infinite_room_snapshot[i].node->estimate().translation()(2);//0;
    ...
    if (!overlapped_infinite_room) {
      ...
      infinite_room_pose_marker.type = visualization_msgs::msg::Marker::CUBE;
      infinite_room_pose_marker.color.r = 0;
      infinite_room_pose_marker.color.g = 10/255.0;
      infinite_room_pose_marker.color.b = 140/255.0;
      infinite_room_pose_marker.color.a = 1.0;
      infinite_room_pose_marker.pose.position.x =
          y_infinite_room_snapshot[i].node->estimate().translation()(0);
      infinite_room_pose_marker.pose.position.y =
  ...
  // room markers
  // std::cout<<"Finite Room Visualization!!!!"<<room_snapshot.size()<<std::endl;
  for (auto& current_room : room_snapshot) {
    current_room.sub_room = false;
    // std::cout<<current_room.id<<std::endl;
  }

  for (auto& current_room : room_snapshot) {
    // std::cout<<current_room.id<<" "<<current_room.sub_room<<std::endl;
    if (current_room.sub_room) continue;
    ...
    room_marker.color.r = 180/255.0;
    room_marker.color.g = 0;
    room_marker.color.b = 40/255.0;
    room_marker.color.a = 1;

    room_marker.pose.position.x = current_room.node->estimate().translation()(0);
    room_marker.pose.position.y = current_room.node->estimate().translation()(1);
    room_marker.pose.position.z = current_room.node->estimate().translation()(2);
    Eigen::Quaterniond quat(current_room.node->estimate().linear());
    ...
    p1.x = current_room.node->estimate().translation()(0);
    p1.y = current_room.node->estimate().translation()(1);
    p1.z = current_room.node->estimate().translation()(2);

    auto found_planex1 = std::find_if(
        x_plane_snapshot.begin(),
        x_plane_snapshot.end(),
        boost::bind(&VerticalPlanes::id, _1) == current_room.plane_x1_id);
    auto found_planex2 = std::find_if(
        x_plane_snapshot.begin(),
        x_plane_snapshot.end(),
        boost::bind(&VerticalPlanes::id, _1) == current_room.plane_x2_id);
    auto found_planey1 = std::find_if(
        y_plane_snapshot.begin(),
        y_plane_snapshot.end(),
        boost::bind(&VerticalPlanes::id, _1) == current_room.plane_y1_id);
    auto found_planey2 = std::find_if(
        y_plane_snapshot.begin(),
        y_plane_snapshot.end(),
        boost::bind(&VerticalPlanes::id, _1) == current_room.plane_y2_id);
    ...
    /* room clusters */
    int cluster_id = 0;
    for (auto& cluster : current_room.cluster_array.markers) {
      cluster.header.frame_id = walls_layer_id;
      cluster.header.stamp = stamp;
      cluster.ns = "room_vertex";
  ...
  for (const auto& floor : floors_vec) {
    if (floor.id != -1) {
      ...
      floor_marker.scale.x = 1;
      floor_marker.scale.y = 1;
      floor_marker.scale.z = 1;
      // plane_marker.points.resize(vert_planes.size());
    ...
    if (edge_wall_dev) {
      ...
      if (abs(v2->estimate().coeffs()(0)) > abs(v2->estimate().coeffs()(1))) {
        ...
        if (found_s_graph_plane != x_vert_planes.end()) {
          ...
          deviation_marker.scale.x = 1.0;
          deviation_marker.scale.y = 1.0;
          deviation_marker.scale.z = 1.0;
...
Eigen::Vector3d GraphVisualizer::compute_hort_plane_centroid(
  ...
  Eigen::Vector3d pt;
  for (auto plane : plane_snapshot) {
    if (plane.id == current_plane_id && !plane.cloud_seg_map->points.empty()) {
      double x = 0, y = 0, z = 0;
      for (int p = 0; p < plane.cloud_seg_map->points.size(); ++p) {
        x += plane.cloud_seg_map->points[p].x;
        y += plane.cloud_seg_map->points[p].y;
