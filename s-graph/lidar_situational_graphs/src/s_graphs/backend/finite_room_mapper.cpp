...
FiniteRoomMapper::FiniteRoomMapper(const rclcpp::Node::SharedPtr node) {
  ...
  infinite_room_dist_threshold =
      node->get_parameter("infinite_room_dist_threshold").get_parameter_value().get<double>();
  room_dist_threshold =
...
bool FiniteRoomMapper::lookup_rooms(
  ...
  float max_overlap_x_inf_room = 0;
  // float matched_x_inf_room_size = 0;
  s_graphs::InfiniteRooms matched_x_infinite_room;
  for (const auto& current_x_infinite_room : x_infinite_rooms) {
    // geometry_msgs::msg::Point length = FiniteRoomMapper::plane_length(current_x_infinite_room.second.cluster_array);

    double infinite_y_min = current_x_infinite_room.second.room_min;
    double infinite_y_max = current_x_infinite_room.second.room_max; 
    // if (current_y_min>infinite_y_min ||current_y_min>infinite_y_max || current_y_max<infinite_y_min || current_y_max<infinite_y_max)
    //   continue;
    ...
    float dist_room_x_inf_room =
        sqrt(pow(room_data.room_center.position.x -
                 ...
                 2) + pow(room_data.room_center.position.y - current_x_infinite_room.second.node->estimate().translation()(1),2));

    geometry_msgs::msg::Point length = FiniteRoomMapper::plane_length(current_x_infinite_room.second.cluster_array);
    float x_overlap = max(0.0,
                        min(room_data.room_center.position.x + room_data.room_length.x/2.0, current_x_infinite_room.second.node->estimate().translation()(0) + length.x/2.0) -
                          max(room_data.room_center.position.x - room_data.room_length.x/2.0, current_x_infinite_room.second.node->estimate().translation()(0) - length.x/2.0));
    float y_overlap = max(0.0,
                        min(room_data.room_center.position.y + room_data.room_length.y/2.0, infinite_y_max) - // current_x_infinite_room.second.node->estimate().translation()(1) + length.y/2.0) -
                          max(room_data.room_center.position.y - room_data.room_length.y/2.0, infinite_y_min)); //current_x_infinite_room.second.node->estimate().translation()(1) - length.y/2.0));
    // std::cout<<"current_x_infinite_room "<<dist_room_x_inf_room<<" "<<current_x_infinite_room.second.id<<" "<<x_overlap<<" "<<y_overlap<<std::endl;
    // if (dist_room_x_inf_room < min_dist_room_x_inf_room && y_overlap>1.0) {
    if (dist_room_x_inf_room < room_dist_threshold && x_overlap*y_overlap > max_overlap_x_inf_room ) {//&& room_data.room_length.y-(infinite_y_max-infinite_y_min)>-0.5 && dist_room_x_inf_room < 1.0) {
      max_overlap_x_inf_room = x_overlap*y_overlap;
      min_dist_room_x_inf_room = dist_room_x_inf_room;
      matched_x_infinite_room = current_x_infinite_room.second;
    }
  }

  float max_overlap_y_inf_room = 0;
  // float matched_y_inf_room_size = 0;
  ...
  for (const auto& current_y_infinite_room : y_infinite_rooms) {
    if ((room_data.y_planes[0].id == current_y_infinite_room.second.plane1_id ||
      ...
      max_overlap_y_inf_room = 100;
      matched_y_infinite_room = current_y_infinite_room.second;
      break;
    }

    double infinite_x_min = current_y_infinite_room.second.room_min;
    double infinite_x_max = current_y_infinite_room.second.room_max;
    
    float dist_room_y_inf_room =
        sqrt(pow(room_data.room_center.position.x -
                     current_y_infinite_room.second.node->estimate().translation()(0),2) 
                     +  pow(room_data.room_center.position.y - current_y_infinite_room.second.node->estimate().translation()(1),2));
    
      geometry_msgs::msg::Point length = FiniteRoomMapper::plane_length(current_y_infinite_room.second.cluster_array);
      float x_overlap = max(0.0,
                          min(room_data.room_center.position.x + room_data.room_length.x/2.0, infinite_x_max) -
                            max(room_data.room_center.position.x - room_data.room_length.x/2.0, infinite_x_min));
      float y_overlap = max(0.0,
                          min(room_data.room_center.position.y + room_data.room_length.y/2.0, current_y_infinite_room.second.node->estimate().translation()(1) + length.y/2.0) -
                            max(room_data.room_center.position.y - room_data.room_length.y/2.0, current_y_infinite_room.second.node->estimate().translation()(1) - length.y/2.0));
    // std::cout<<"current_y_infinite_room "<<dist_room_y_inf_room<<" "<<current_y_infinite_room.second.id<<" "<<x_overlap<<" "<<y_overlap<<std::endl;

    // if (dist_room_y_inf_room < min_dist_room_y_inf_room && x_overlap>1.0) {
    if (dist_room_y_inf_room < room_dist_threshold && x_overlap*y_overlap > max_overlap_y_inf_room) {//  && room_data.room_length.x-(infinite_x_max-infinite_x_min)>-0.5 && dist_room_y_inf_room < 1.0) {
      max_overlap_y_inf_room = x_overlap*y_overlap;
  ...
  // std::cout<<"Check x inf room and y inf room can merged into one finite room---"<<std::endl;
  // std::cout<<"max_overlap_x_inf_room "<<max_overlap_x_inf_room<<" max_overlap_y_inf_room "<<max_overlap_y_inf_room<<std::endl;
  
  // if (min_dist_room_y_inf_room < 1.0 && min_dist_room_x_inf_room < 1.0) {
  if (max_overlap_x_inf_room>0 && max_overlap_y_inf_room>0 
   && min_dist_room_y_inf_room < infinite_room_dist_threshold && min_dist_room_x_inf_room < infinite_room_dist_threshold) {
    ...
    if (!is_x1_plane_aligned_w_y((found_x_plane1->second).cloud_seg_map, (found_y_plane1->second).cloud_seg_map) ||
        !is_x1_plane_aligned_w_y((found_x_plane1->second).cloud_seg_map, (found_y_plane2->second).cloud_seg_map)) {
      invalid = true;
    } if (!is_x2_plane_aligned_w_y((found_x_plane2->second).cloud_seg_map, (found_y_plane1->second).cloud_seg_map) ||
        !is_x2_plane_aligned_w_y((found_x_plane2->second).cloud_seg_map, (found_y_plane2->second).cloud_seg_map)) {
      invalid = true;
    } if (!is_y1_plane_aligned_w_x((found_y_plane1->second).cloud_seg_map, (found_x_plane1->second).cloud_seg_map) ||
        !is_y1_plane_aligned_w_x((found_y_plane1->second).cloud_seg_map, (found_x_plane2->second).cloud_seg_map)) {
      invalid = true;
    } if (!is_y2_plane_aligned_w_x((found_y_plane2->second).cloud_seg_map, (found_x_plane1->second).cloud_seg_map) ||
        !is_y2_plane_aligned_w_x((found_y_plane2->second).cloud_seg_map, (found_x_plane2->second).cloud_seg_map)) {
      invalid = true;
    }
    if (invalid) {
      matched = true;
      ...
      std::cout<<"alignment failed! Factoring new finite room"<<std::endl;
    }
  } else if (max_overlap_x_inf_room>0 && max_overlap_y_inf_room==0 && min_dist_room_x_inf_room < infinite_room_dist_threshold) {
    ...
    remove_mapped_infinite_room(PlaneUtils::plane_class::X_VERT_PLANE,
                                ...
                                x_infinite_rooms,
                                y_infinite_rooms);
  } else if (max_overlap_x_inf_room==0 && max_overlap_y_inf_room>0 && min_dist_room_y_inf_room < infinite_room_dist_threshold) {
...
bool FiniteRoomMapper::factor_rooms(
  ...
  if ((rooms_vec.empty() || room_data_association == -1)) {
    ...
    std::cout<<"room_length "<<room_length.x<<" "<<room_length.y<<std::endl;
    det_room.room_x_max = room_center.translation().x()+room_length.x/2;
    det_room.room_x_min = room_center.translation().x()-room_length.x/2;
    det_room.room_y_max = room_center.translation().y()+room_length.y/2;
    det_room.room_y_min = room_center.translation().y()-room_length.y/2;
    
    // keyframes
    // if (room_length.x>room_length.y) {
    //   det_room.room_type = "y";
    //   get_room_keyframes(det_room, x_vert_planes, y_vert_planes, keyframes,
    //                       "y", det_room.room_x_min, det_room.room_x_max, roompart_length);
    // } else {
    //   det_room.room_type = "x";
    //   get_room_keyframes(det_room, x_vert_planes, y_vert_planes, keyframes,
    //                       "x", det_room.room_y_min, det_room.room_y_max, roompart_length);
    // }
    
    det_room.room_keyframes = get_room_keyframes(det_room, x_vert_planes, y_vert_planes, keyframes);
    det_room.new_roompart_keyframes.insert({0,{0,0,det_room.room_keyframes}});
    ...
    //   get_room_keyframes(det_room, x_vert_planes, y_vert_planes, keyframes,
    //                               "x", det_room.room_y_min, det_room.room_y_max, roompart_length);
    // else get_room_keyframes(det_room, x_vert_planes, y_vert_planes, keyframes,
    //                               "y", det_room.room_x_min, det_room.room_x_max, roompart_length);
    
    rooms_vec[room_data_association].room_x_max = max(rooms_vec[room_data_association].room_x_max, room_center.translation().x()+room_length.x/2);
    rooms_vec[room_data_association].room_x_min = min(rooms_vec[room_data_association].room_x_min, room_center.translation().x()-room_length.x/2);
    rooms_vec[room_data_association].room_y_max = max(rooms_vec[room_data_association].room_y_max, room_center.translation().y()+room_length.y/2);
    rooms_vec[room_data_association].room_y_min = min(rooms_vec[room_data_association].room_y_min, room_center.translation().y()-room_length.y/2);

    for (auto &kf: rooms_vec[room_data_association].room_keyframes) {
      std::get<2>(rooms_vec[room_data_association].old_roompart_keyframes[0]).insert(kf); }
    det_room.room_keyframes = get_room_keyframes(det_room, x_vert_planes, y_vert_planes, keyframes);
    rooms_vec[room_data_association].new_roompart_keyframes.clear();
    rooms_vec[room_data_association].new_roompart_keyframes.insert({0,{0,0,det_room.room_keyframes}});

    mergeKeyframes(rooms_vec[room_id].room_keyframes, det_room.room_keyframes);
    
    /*update the cluster array */
    mergeMarkerArrays(rooms_vec[room_data_association], cluster_array);
    setRoomCenter(rooms_vec[room_data_association], room_center);
    // setLargerPlanes(rooms_vec[room_data_association], det_room);
    ...
    if (detected_mapped_plane_pairs[2].first.id !=
      ...
      if (!MapperUtils::check_plane_ids(
          ...
          && PlaneUtils::plane_dot_product(detected_mapped_plane_pairs[2].first.plane_node
                , detected_mapped_plane_pairs[2].second.plane_node)> 0.9) {
        auto edge_planes = graph_slam->add_2planes_edge(
    ...
    if (detected_mapped_plane_pairs[3].first.id !=
      ...
      if (!MapperUtils::check_plane_ids(
          ...
          && PlaneUtils::plane_dot_product(detected_mapped_plane_pairs[3].first.plane_node
                , detected_mapped_plane_pairs[3].second.plane_node)> 0.9) {
        auto edge_planes = graph_slam->add_2planes_edge(
...
int FiniteRoomMapper::associate_rooms(
  ...
  for (const auto& room : rooms_vec) {
    ...
    float x_overlap = max(0.0,
                        min(room_center.translation().x() + room_length.x/2.0, room.second.room_x_max) -
                          max(room_center.translation().x()- room_length.x/2.0, room.second.room_x_min));
    float y_overlap = max(0.0,
                        min(room_center.translation().y() + room_length.y/2.0, room.second.room_y_max) - 
                          max(room_center.translation().y() - room_length.y/2.0, room.second.room_y_min)); 
    ...
    std::cout<<"room association with id: "<<room.second.id<<" dist: "<<dist<<" overlap: "<<x_overlap<<" "<<y_overlap<<std::endl;
    if (dist < room_dist_threshold && 
       ( (x_plane1_min_segment && x_plane2_min_segment && y_plane1_min_segment && y_plane2_min_segment)
        || (x_overlap*y_overlap>max_overlap_room && current_detected_mapped_plane_pairs.size()>=2)) ) {
      max_overlap_room = x_overlap*y_overlap;
      min_dist = dist;
  ...
  if (max_overlap_room < 1) data_association = -1;
...
void FiniteRoomMapper::map_room_from_existing_infinite_rooms(
  ...
  if ((rooms_vec.empty() || room_data_association == -1)) {
    ...
    det_room.room_x_max = max(room_center.translation().x()+det_room_data.room_length.x/2, matched_y_infinite_room.room_max);
    det_room.room_x_min = min(room_center.translation().x()-det_room_data.room_length.x/2, matched_y_infinite_room.room_min);
    det_room.room_y_max = max(room_center.translation().y()+det_room_data.room_length.y/2, matched_x_infinite_room.room_max);
    det_room.room_y_min = min(room_center.translation().y()-det_room_data.room_length.y/2, matched_x_infinite_room.room_min);

    for (auto &kf: matched_x_infinite_room.room_keyframes) {
      std::get<2>(det_room.old_roompart_keyframes[0]).insert(kf); }
    for (auto &kf: matched_y_infinite_room.room_keyframes) {
      std::get<2>(det_room.old_roompart_keyframes[0]).insert(kf); }
    det_room.room_keyframes = get_room_keyframes(det_room, x_vert_planes, y_vert_planes, keyframes);

    mergeKeyframes(det_room.room_keyframes, matched_y_infinite_room.room_keyframes);
    mergeKeyframes(det_room.room_keyframes, matched_x_infinite_room.room_keyframes);

    det_room.new_roompart_keyframes.insert({0,{0,0,det_room.room_keyframes}});
    // std::cout<<"det_room "<< det_room.new_roompart_keyframes.size()<<" "<< det_room.old_roompart_keyframes.size()<<std::endl;
    // std::cout<<"det_room_data.cluster_array "<<det_room_data.cluster_array.markers.size()<<" "<<det_room_data.cluster_array.markers[0].points.size()<<std::endl;
    mergeMarkerArrays(det_room, det_room_data.cluster_array);
    mergeMarkerArrays(det_room, matched_x_infinite_room.cluster_array);
    mergeMarkerArrays(det_room, matched_y_infinite_room.cluster_array);
    // std::cout<<"det_room_data.cluster_array "<<det_room_data.cluster_array.markers.size()<<" "<<det_room_data.cluster_array.markers[0].points.size()<<std::endl;

    // setRoomCenter(det_room, room_center);
...
void FiniteRoomMapper::map_room_from_existing_x_infinite_room(
  ...
  if ((rooms_vec.empty() || room_data_association == -1)) {
    ...
    // update_old_roompart_keyframes(matched_x_infinite_room.room_keyframes,
    //                 "x", matched_x_infinite_room.room_min,
    //                 matched_x_infinite_room.room_max, roompart_length,
    //                 det_room.old_roompart_keyframes);
    ...
    det_room.cluster_array = det_room_data.cluster_array;

    det_room.room_x_max = room_center.translation().x()+det_room_data.room_length.x/2;
    det_room.room_x_min = room_center.translation().x()-det_room_data.room_length.x/2;
    det_room.room_y_max = max(room_center.translation().y()+det_room_data.room_length.y/2,matched_x_infinite_room.room_max);
    det_room.room_y_min = min(room_center.translation().y()-det_room_data.room_length.y/2,matched_x_infinite_room.room_min);
    ...
    //                           "x", det_room.room_y_min, det_room.room_y_max, roompart_length);
    // std::cout<<"det_room "<< det_room.new_roompart_keyframes.size()<<" "<< det_room.old_roompart_keyframes.size()<<std::endl;
    // mergeKeyframes(det_room.room_keyframes, matched_x_infinite_room.room_keyframes, keyframes);

    // double x_length = det_room.room_y_max - det_room.room_y_min;
    // double y_length = det_room.room_x_max - det_room.room_x_min;

    // if (y_length>x_length){
    //   det_room.room_type="x";
    //   update_old_roompart_keyframes(matched_x_infinite_room.room_keyframes,
    //                 "x", det_room.room_y_min, det_room.room_y_max, roompart_length,
    //                 det_room.old_roompart_keyframes); //old
    //   get_room_keyframes(det_room, x_vert_planes, y_vert_planes, keyframes,
    //                               "x", det_room.room_y_min, det_room.room_y_max, roompart_length);
    //             // new 
    // }
...
void FiniteRoomMapper::map_room_from_existing_y_infinite_room(
  ...
  if ((rooms_vec.empty() || room_data_association == -1)) {
    ...
    // update_old_roompart_keyframes(matched_y_infinite_room.room_keyframes,
    //                 "y", matched_y_infinite_room.room_min, matched_y_infinite_room.room_max, roompart_length,
    //                 det_room.old_roompart_keyframes);
    ...
    det_room.room_y_max = room_center.translation().y()+det_room_data.room_length.y/2;
    det_room.room_y_min = room_center.translation().y()-det_room_data.room_length.y/2;
    det_room.room_x_max = max(det_room.room_x_max, matched_y_infinite_room.room_max);
    det_room.room_x_min = min(det_room.room_x_min, matched_y_infinite_room.room_min);
