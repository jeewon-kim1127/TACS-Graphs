...
InfiniteRoomGraphGenerator::InfiniteRoomGraphGenerator(rclcpp::Node::SharedPtr node) {}

InfiniteRoomGraphGenerator::~InfiniteRoomGraphGenerator() {}


std::map<int, KeyFrame::Ptr> InfiniteRoomGraphGenerator::get_keyframes_inside_infinite_room(
    const InfiniteRooms& current_room,
  ...
  if (current_room.node != nullptr) {
    room_keyframes =
        get_infinite_room_keyframes(current_room, vert_planes, keyframes);
  }
...
std::vector<const s_graphs::VerticalPlanes*> get_room_planes(
    const InfiniteRooms& current_room,
    const std::unordered_map<int, VerticalPlanes>& vert_planes) {
  auto planes = obtain_planes_from_infinite_room(current_room, vert_planes);
  return planes;
}

void InfiniteRoomGraphGenerator::generate_local_graph(
...
void InfiniteRoomGraphGenerator::update_room_graph(
    const InfiniteRooms room,
