...
OldFiniteRoomMapper::OldFiniteRoomMapper(const rclcpp::Node::SharedPtr node) {
  node_obj = node;
  room_information =
  ...
  use_old_ablation = node->get_parameter("use_old_ablation")
...
OldFiniteRoomMapper::~OldFiniteRoomMapper() {};

bool OldFiniteRoomMapper::lookup_rooms(
  ...
  double max_overlap_x_inf_room = 0.0;
  ...
  for (const auto& current_x_infinite_room : x_infinite_rooms) {
    ...
    if (use_old_ablation){
      dist_room_x_inf_room = sqrt(pow(room_data.room_center.position.x -
                     current_x_infinite_room.second.node->estimate().translation()(0),
                 2));
      geometry_msgs::msg::Point length = OldFiniteRoomMapper::plane_length(current_x_infinite_room.second.cluster_array);
      ...
      float y_overlap = max(0.0,
                          min(room_data.room_center.position.y + room_data.room_length.y/2.0, current_x_infinite_room.second.node->estimate().translation()(1) + length.y/2.0) -
                            max(room_data.room_center.position.y - room_data.room_length.y/2.0, current_x_infinite_room.second.node->estimate().translation()(1) - length.y/2.0));
      if (dist_room_x_inf_room < room_dist_threshold && x_overlap*y_overlap > max_overlap_x_inf_room ) {
  ...
  double max_overlap_y_inf_room = 0.0;
  ...
  for (const auto& current_y_infinite_room : y_infinite_rooms) {
    ...
    if (use_old_ablation){
      dist_room_y_inf_room = sqrt(pow(room_data.room_center.position.y -
                     current_y_infinite_room.second.node->estimate().translation()(1),
                 2));
      geometry_msgs::msg::Point length = OldFiniteRoomMapper::plane_length(current_y_infinite_room.second.cluster_array);
      float x_overlap = max(0.0,
                          min(room_data.room_center.position.x + room_data.room_length.x/2.0, current_y_infinite_room.second.node->estimate().translation()(0) + length.x/2.0) -
                            max(room_data.room_center.position.x - room_data.room_length.x/2.0, current_y_infinite_room.second.node->estimate().translation()(0) - length.x/2.0));
      float y_overlap = max(0.0,
                          min(room_data.room_center.position.y + room_data.room_length.y/2.0, current_y_infinite_room.second.node->estimate().translation()(1) + length.y/2.0) -
                            max(room_data.room_center.position.y - room_data.room_length.y/2.0, current_y_infinite_room.second.node->estimate().translation()(1) - length.y/2.0)); //current_x_inf
      if (dist_room_y_inf_room < room_dist_threshold && x_overlap*y_overlap > max_overlap_y_inf_room ) {
        max_overlap_y_inf_room = x_overlap*y_overlap;
        min_dist_room_y_inf_room = dist_room_y_inf_room;
        matched_y_infinite_room = current_y_infinite_room.second;
      }
    } else {
      dist_room_y_inf_room = sqrt(pow(room_data.room_center.position.x -
  ...
  if ((!use_old_ablation && min_dist_room_y_inf_room < 1.0 && min_dist_room_x_inf_room < 1.0) 
      || (use_old_ablation && max_overlap_x_inf_room>0 && max_overlap_y_inf_room>0)) {
    ...
    remove_mapped_infinite_room(PlaneUtils::plane_class::Y_VERT_PLANE,
                                ...
                                x_infinite_rooms,
                                y_infinite_rooms);
  } else if ((!use_old_ablation && min_dist_room_x_inf_room < 1.0 && min_dist_room_y_inf_room > 1.0)
    || (use_old_ablation && max_overlap_x_inf_room>0 && max_overlap_y_inf_room==0)) {
    ...
    remove_mapped_infinite_room(PlaneUtils::plane_class::X_VERT_PLANE,
                                ...
                                x_infinite_rooms,
                                y_infinite_rooms);
  } else if ((!use_old_ablation && min_dist_room_y_inf_room < 1.0 && min_dist_room_x_inf_room > 1.0)
      || (use_old_ablation && max_overlap_x_inf_room==0 && max_overlap_y_inf_room>0)) {
...
bool OldFiniteRoomMapper::factor_rooms(
    std::shared_ptr<GraphSLAM>& graph_slam,
  ...
  if ((rooms_vec.empty() || room_data_association == -1)) {
    ...
    if (!use_old_ablation) rooms_vec[room_data_association].cluster_array = cluster_array;
    else {
...
int OldFiniteRoomMapper::associate_rooms(
    const Eigen::Isometry3d& room_center,
    const geometry_msgs::msg::Point &room_length,
  ...
  for (const auto& room : rooms_vec) {
    ...
    if (!use_old_ablation && dist < min_dist &&
    ...
    else if (use_old_ablation && dist < room_dist_threshold && x_overlap*y_overlap>max_overlap_room &&
           (x_plane1_min_segment && x_plane2_min_segment && y_plane1_min_segment && y_plane2_min_segment)) {
...
bool OldFiniteRoomMapper::check_room_ids(
...
void OldFiniteRoomMapper::map_room_from_existing_infinite_rooms(
    std::shared_ptr<GraphSLAM>& graph_slam,
...
void OldFiniteRoomMapper::map_room_from_existing_x_infinite_room(
    std::shared_ptr<GraphSLAM>& graph_slam,
    const situational_graphs_msgs::msg::RoomData& det_room_data,
    const s_graphs::InfiniteRooms& matched_x_infinite_room,
    const Eigen::Isometry3d& room_center,
...
void OldFiniteRoomMapper::map_room_from_existing_y_infinite_room(
    std::shared_ptr<GraphSLAM>& graph_slam,
    const situational_graphs_msgs::msg::RoomData& det_room_data,
    const s_graphs::InfiniteRooms& matched_y_infinite_room,
    const Eigen::Isometry3d& room_center,
...
void OldFiniteRoomMapper::remove_mapped_infinite_room(
    const int plane_type,
    std::shared_ptr<GraphSLAM>& graph_slam,
