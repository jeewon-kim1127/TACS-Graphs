...
int PlaneMapper::add_planes_to_graph(
  ...
  else if (fabs(det_plane_map_frame.coeffs()(1)) > fabs(det_plane_map_frame.coeffs()(0)) &&
           fabs(det_plane_map_frame.coeffs()(1)) > fabs(det_plane_map_frame.coeffs()(2)))
    plane_type = PlaneUtils::plane_class::Y_VERT_PLANE;
  else if (fabs(det_plane_map_frame.coeffs()(2)) > fabs(det_plane_map_frame.coeffs()(0)) &&
           fabs(det_plane_map_frame.coeffs()(2)) > fabs(det_plane_map_frame.coeffs()(1)))
...
int PlaneMapper::factor_planes(std::shared_ptr<GraphSLAM>& graph_slam,
  ...
  if (use_point_to_plane) {
    Eigen::Matrix<double, 1, 1> information(1.0);
  ...
  switch (plane_type) {
    ...
    case PlaneUtils::plane_class::Y_VERT_PLANE: {
      for (const auto& y_vert_plane : y_vert_planes) {
        // float dist = fabs(det_plane.coeffs()(3) - y_vert_plane.second.plane.coeffs()(3));
