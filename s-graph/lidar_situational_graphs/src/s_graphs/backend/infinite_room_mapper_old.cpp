...
OldInfiniteRoomMapper::OldInfiniteRoomMapper(const rclcpp::Node::SharedPtr node) {
  node_obj = node;
...
OldInfiniteRoomMapper::~OldInfiniteRoomMapper() {}

bool OldInfiniteRoomMapper::lookup_infinite_rooms(
  ...
  if (plane_type == PlaneUtils::plane_class::X_VERT_PLANE) {
    ...
    for (const auto& current_room : rooms_vec) {
      ...
      float dist_x_inf_room_room =
          sqrt(pow(room_data.room_center.position.x -
                       current_room.second.node->estimate().translation()(0), 2) +
               pow(room_data.room_center.position.y -
                       current_room.second.node->estimate().translation()(1), 2));
    ...
    if (min_dist_y_inf_room_room < 1.0) {
      std::cout << "Room already exists in the given location, not inserting a y infinite_room"
                << std::endl;
      room_id = matched_room.id;
      matched = true;
...
bool OldInfiniteRoomMapper::factor_infinite_rooms(
    std::shared_ptr<GraphSLAM>& graph_slam,
    const int plane_type,
  ...
  if (plane_type == PlaneUtils::plane_class::X_VERT_PLANE) {
    ...
    if ((x_infinite_rooms.empty() || room_data_association == -1)) {
      ...
      if (!use_old_ablation) x_infinite_rooms[room_data_association].cluster_array = cluster_array;
      else {
        ...
        x_infinite_rooms[room_data_association].room_max = max(x_infinite_rooms[room_data_association].room_max, room_center.translation().x()+room_length.x/2);
        x_infinite_rooms[room_data_association].room_min = min(x_infinite_rooms[room_data_association].room_min, room_center.translation().x()-room_length.x/2);
  ...
  if (plane_type == PlaneUtils::plane_class::Y_VERT_PLANE) {
    ...
    if ((y_infinite_rooms.empty() || room_data_association == -1)) {
      ...
      if (!use_old_ablation) y_infinite_rooms[room_data_association].cluster_array = cluster_array;
      else {
        ...
        y_infinite_rooms[room_data_association].room_max = max(y_infinite_rooms[room_data_association].room_max, room_center.translation().x()+room_length.x/2);
        y_infinite_rooms[room_data_association].room_min = min(y_infinite_rooms[room_data_association].room_min, room_center.translation().x()-room_length.x/2);
      }
...
int OldInfiniteRoomMapper::associate_infinite_rooms(
    const int& plane_type,
    const Eigen::Isometry3d& room_center,
    const geometry_msgs::msg::Point &room_length,
    const visualization_msgs::msg::MarkerArray& cluster_array,
  ...
  if (plane_type == PlaneUtils::plane_class::X_VERT_PLANE) {
    for (const auto& x_inf_room : x_infinite_rooms) {
      ...
      geometry_msgs::msg::Point length = OldInfiniteRoomMapper::plane_length(x_inf_room.second.cluster_array);
      ...
      float y_overlap = max(0.0,
                            min(room_center.translation()(1) + room_length.y/2.0, x_inf_room.second.node->estimate().translation()(1) + length.y/2.0) -
                             max(room_center.translation()(1) - room_length.y/2.0, x_inf_room.second.node->estimate().translation()(1) - length.y/2.0));
      if (use_old_ablation){
      ...
      if (!use_old_ablation && dist < min_dist && (plane1_min_segment && plane2_min_segment)) {
        min_dist = dist;
      ...
      if (use_old_ablation && dist<infinite_room_dist_threshold 
            && x_inf_room.second.id<min_id && x_overlap>1.0 && y_overlap>1.0 
  ...
  if (plane_type == PlaneUtils::plane_class::Y_VERT_PLANE) {
    for (const auto& y_inf_room : y_infinite_rooms) {
      ...
      geometry_msgs::msg::Point length = OldInfiniteRoomMapper::plane_length(y_inf_room.second.cluster_array);
      float x_overlap = max(0.0,
                            min(room_center.translation()(0) + room_length.x/2.0, y_inf_room.second.node->estimate().translation()(0) + length.x/2.0) -
                             max(room_center.translation()(0) - room_length.x/2.0, y_inf_room.second.node->estimate().translation()(0) - length.x/2.0));
      float y_overlap = max(0.0,
      ...
      if (use_old_ablation && dist<infinite_room_dist_threshold && y_inf_room.second.id<min_id && x_overlap>1.0 && y_overlap>1.0 && (plane1_min_segment || plane2_min_segment)) {      
        min_dist = dist;
        data_association = y_inf_room.second.id;
        detected_mapped_plane_pairs = current_detected_mapped_plane_pairs;
        RCLCPP_DEBUG(
  ...
  // min_dist);
  if (!use_old_ablation && min_dist > infinite_room_dist_threshold) data_association = -1;
  if (data_association>30000) data_association = -1;
  return data_association;
}

bool OldInfiniteRoomMapper::check_infinite_room_ids(
    const int plane_type,
