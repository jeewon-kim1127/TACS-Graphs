...
std::vector<g2o::VertexSE3*> GraphUtils::copy_graph_edges(
  ...
  for (g2o::HyperGraph::EdgeSet::iterator it =
    ...
    if (!edge_se3 ||  edge_se3->vertices().size()<2) continue;
    ...
    if (!edge_se3_plane ||  edge_se3_plane->vertices().size()<2) continue;
    if (edge_se3_plane) {
    ...
    if (!edge_room_2planes ||  edge_room_2planes->vertices().size()<4) continue;
    ...
    if (!edge_room_4planes ||  edge_room_4planes->vertices().size()<5) continue;
    ...
    if (!edge_2planes ||  edge_2planes->vertices().size()<2) continue;
    if (edge_2planes) {
    ...
    /////////////// EdgePlaneIdentity
    g2o::EdgePlaneIdentity* edge_plane_identity = dynamic_cast<g2o::EdgePlaneIdentity*>(e);
    if (!edge_plane_identity ||  edge_plane_identity->vertices().size()<2) continue;
    if (edge_plane_identity) {
      g2o::VertexPlane* v1 = dynamic_cast<g2o::VertexPlane*>(
          compressed_graph->graph->vertices().at(edge_plane_identity->vertices()[0]->id()));
      g2o::VertexPlane* v2 = dynamic_cast<g2o::VertexPlane*>(
          compressed_graph->graph->vertices().at(edge_plane_identity->vertices()[1]->id()));
      auto edge = compressed_graph->copy_plane_identity_edge(edge_plane_identity, v1, v2);
      compressed_graph->add_robust_kernel(edge, "Huber", 1.0);
      continue;
    }

    g2o::EdgeWall2Planes* edge_wall_2planes = dynamic_cast<g2o::EdgeWall2Planes*>(e);
    if (!edge_wall_2planes ||  edge_wall_2planes->vertices().size()<2) continue;
    ...
    if (!edge_floor_room ||  edge_floor_room->vertices().size()<2) continue;
    if (edge_floor_room) {
