...
// void PlaneUtils::mergePlanes(
//     const s_graphs::InfiniteRooms &room, 
//     std::unordered_map<int, VerticalPlanes>& vert_planes,
//     const situational_graphs_msgs::msg::PlaneData &plane1,
//     const situational_graphs_msgs::msg::PlaneData &plane2)
// {
//   // auto found_mapped_plane1 = vert_planes.find(room.plane1_id);
//   // auto found_mapped_plane2 = vert_planes.find(room.plane2_id);

//   if (plane_dot_product(plane1, room.plane1)) {
//     for (auto pt: plane1.cloud_seg_map->points)
//       (vert_planes->second[room.plane1_id]).cloud_seg_map->points.push_back(pt);
//   } else if (plane_dot_product(plane2, room.plane1)) {
//     for (auto pt: plane2.cloud_seg_map->points)
//       (vert_planes->second[room.plane1_id]).cloud_seg_map->points.push_back(pt);
//   }
//   if (plane_dot_product(plane2, room.plane2)) {
//     for (auto pt: plane2.cloud_seg_map->points)
//       (vert_planes->second[room.plane2_id]).cloud_seg_map->points.push_back(pt);
//   } else if (plane_dot_product(plane1, room.plane2)) {
//     for (auto pt: plane1.cloud_seg_map->points)
//       (vert_planes->second[room.plane2_id]).cloud_seg_map->points.push_back(pt);
//   }
//   return;
...
float PlaneUtils::width_between_planes(
  ...
  // if (abs(plane1_eigen.dot(plane2_eigen))<0.5){
...
float PlaneUtils::width_between_planes(
    const g2o::Plane3D& plane1,
    const g2o::Plane3D& plane2) {
  Eigen::Vector3d vec;
  Eigen::Vector4d plane1_4eigen, plane2_4eigen;
  plane1_4eigen = plane1.toVector().transpose();
  plane2_4eigen = plane1.toVector().transpose();

  Eigen::Vector3d plane1_eigen, plane2_eigen;
  plane1_eigen << plane1_4eigen(0), plane1_4eigen(1), plane1_4eigen(2);
  plane2_eigen << plane2_4eigen(0), plane2_4eigen(1), plane2_4eigen(2);
  float size = 0;

  if (fabs( plane1_4eigen(3)) > fabs(plane2_4eigen(3)))
    vec = fabs(plane1_4eigen(3)) * plane1_eigen - fabs(plane2_4eigen(3)) * plane2_eigen;
  else if (fabs(plane2_4eigen(3)) > fabs(plane1_4eigen(3)))
    vec = fabs(plane2_4eigen(3)) * plane2_eigen - fabs(plane1_4eigen(3)) * plane1_eigen;
...
bool PlaneUtils::check_point_neighbours(
  ...
  for (std::size_t i = 0; i < cloud_1->points.size(); ++i) {
    ...
    // std::cout<<"check_point_neighbours "<<point_count<<std::endl;
    if (point_count > 100) {
...
// bool PlaneUtils::plane_dot_product(
//        const situational_graphs_msgs::msg::PlaneData& plane1,
//        g2o::VertexPlane* plane2) {
//   Eigen::Vector3d normal1(plane1.nx,plane1.ny, plane1.nz, plane1.d);
//   Eigen::Vector4d coeffs2 = plane2->estimate().coeffs();
//   Eigen::Vector3d normal2(coeffs2[0], coeffs2[1], coeffs2[2]);
//   double dot_product = normal1.dot(normal2);
//   std::cout << "dot product : " << dot_product << std::endl;
...
geometry_msgs::msg::Pose PlaneUtils::extract_infite_room_center(
  ...
  cluster_center(0) = (p1.x + p2.x)/2;
  cluster_center(1) = (p1.y + p2.y)/2;
...
geometry_msgs::msg::Pose PlaneUtils::extract_infinite_room_center(
    int plane_type,
    pcl::PointXY p1,
    pcl::PointXY p2,
    situational_graphs_msgs::msg::PlaneData plane1,
