...
pathplan::subPath pathplan::pathplanning(cv::Mat map, cv::Point src, cv::Point dst)
{
    ...
    if (save_time) {
        std::cout << "duration to path planning: " << 
            boost::format("%.3f") % (t2 -t1).seconds() << std::endl;
    }
...
void pathplan::procMap()
{
    ...
    // cv::imshow("output_image", m_occumapCv.cvMapForPlan);
    // cv::waitKey(0);
